  ...
  return (
    <div className="relative w-full">
      <input
        ...
        className="w-full border p-3 sm:p-4 rounded-xl bg-slate-50 text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none transition-all uppercase"
      />
      ...
      {avisoInativo && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-red-950/95 backdrop-blur-sm animate-fadeIn p-4">
          <div className="bg-white p-8 rounded-[2rem] shadow-2xl text-center max-w-md w-full border-4 border-red-500 animate-bounce">
            ...
            <p className="text-slate-600 font-bold mb-6 text-sm">Você está inativo há quase 2 horas.<br/>Sua sessão será encerrada em:</p>
        ...
        <div className="max-w-7xl mx-auto p-4 flex flex-col gap-4">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-4">
            ...
            <div className="flex flex-col gap-1 w-full md:w-auto">
              ...
              <div className="flex gap-1 justify-between sm:justify-end overflow-x-auto hide-scrollbar pb-1">
      ...
      <main className="max-w-7xl mx-auto p-4 lg:p-8">
              ...
              <div className="flex flex-col gap-3">
                {rankingTop5.map((pessoa: any, index: number) => (
                  <div key={pessoa[0]} className="w-full">
                    <div className="flex justify-between text-[9px] sm:text-[10px] font-bold text-slate-600 mb-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div className="bg-white p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] shadow-sm border-l-[8px] sm:border-l-[12px] border-blue-500 flex justify-between items-center transform hover:scale-[1.02] transition-transform duration-300">
               <div>
                  ...
                  <p className="text-xl sm:text-3xl font-black text-slate-900">R$ {totalNovoSEI.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
               </div>
            ...
            <div className="bg-white p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] shadow-sm border-l-[8px] sm:border-l-[12px] border-emerald-500 flex justify-between items-center transform hover:scale-[1.02] transition-transform duration-300">
               <div>
                  ...
                  <p className="text-xl sm:text-3xl font-black text-slate-900">R$ {totalNovoSalario.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
               </div>
          ...
          <div className="mb-8 bg-white p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] shadow-sm border border-slate-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {tabelasPendentesArray.map((tabela: any, idx: number) => (
                <div key={idx} className="bg-amber-50 border border-amber-200 p-4 sm:p-5 rounded-2xl sm:rounded-3xl shadow-sm flex flex-col justify-between relative overflow-hidden">
                   <div className="absolute top-0 left-0 w-1.5 sm:w-2 h-full bg-amber-400"></div>
                   <div className="pl-3">
                     <div className="flex justify-between items-start mb-2">
                       ...
                       <button onClick={() => excluirRelatorioGerado(tabela.ids)} className="text-amber-300 hover:text-red-500 transition-colors text-base sm:text-lg" title="Desfazer/Excluir Tabela">🗑️</button>
                     </div>
                     <h3 className="text-xl sm:text-2xl font-black text-slate-900 mt-2 mb-1">R$ {tabela.valorTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
                     <p className="text-[9px] sm:text-[10px] font-medium text-amber-700">{tabela.ids.length} diárias na tabela</p>
                   </div>
                   <div className="pl-3 mt-4 flex flex-col gap-2">
                      <button onClick={() => baixarRelatorioAntigo(tabela.metodo, tabela.ids, tabela.data)} className="bg-white border border-amber-200 hover:bg-amber-100 text-amber-700 py-2 sm:py-2.5 rounded-xl text-[9px] sm:text-[10px] font-bold uppercase transition-all shadow-sm">
                        📥 Refazer Download
                      </button>
                      {tabela.comprovante_url ? (
                         <a href={tabela.comprovante_url} target="_blank" rel="noopener noreferrer" className="bg-slate-800 hover:bg-slate-900 text-white text-center py-2 sm:py-2.5 rounded-xl text-[9px] sm:text-[10px] font-bold uppercase transition-all shadow-sm">
                           📄 Ver Cópia Salva
                         </a>
                      ) : (
                         <label className="bg-white border border-dashed border-amber-300 hover:border-amber-500 text-amber-700 py-2 sm:py-2.5 rounded-xl text-[9px] sm:text-[10px] font-bold uppercase transition-all cursor-pointer flex items-center justify-center gap-2">
                           {uploadingTabela === tabela.key ? '⏳ Enviando...' : '📤 Anexar Backup'}
                      ...
                      <button onClick={() => marcarTabelaPaga(tabela.ids)} className="w-full bg-amber-500 hover:bg-amber-600 text-white font-black py-2.5 sm:py-3 rounded-xl uppercase text-[9px] sm:text-[10px] tracking-widest shadow-sm transition-all active:scale-95 mt-1">
                        ✓ Marcar Tabela Paga
                      </button>
                   </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          <aside className="lg:col-span-4">
            <div className="bg-white p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] shadow-sm border border-slate-100 lg:sticky top-40">
              ...
              <form onSubmit={cadastrarDiaria} className="flex flex-col gap-3">
                ...
                <input name="adolescente_nome" placeholder="Adolescente / Motivo" className="border p-3 sm:p-4 rounded-xl bg-slate-50 text-sm font-medium uppercase outline-none focus:ring-2 focus:ring-blue-500" onInput={(e) => e.currentTarget.value = e.currentTarget.value.toUpperCase()} required />
                <div className="grid grid-cols-2 gap-2">
                  <input name="data" type="date" className="border p-3 sm:p-4 rounded-xl bg-slate-50 text-xs font-bold text-slate-600 outline-none focus:ring-2 focus:ring-blue-500" required />
                  <input name="valor" type="number" step="0.01" placeholder="R$ Valor" className="border p-3 sm:p-4 rounded-xl bg-slate-50 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500" required />
                </div>
                {metodoSelecionado === 'CONTA SALARIO' && <input name="quantidade" type="number" placeholder="Qtd. Diárias" className="border-2 border-emerald-100 p-3 sm:p-4 rounded-xl bg-emerald-50 text-sm font-medium text-emerald-800 outline-none focus:ring-2 focus:ring-emerald-500" />}
                ...
                <select value={metodoSelecionado} onChange={(e) => setMetodoSelecionado(e.target.value)} className="border p-3 sm:p-4 rounded-xl bg-slate-100 text-sm font-bold outline-none cursor-pointer focus:ring-2 focus:ring-blue-500"><option value="SEI">SISTEMA SEI</option><option value="CONTA SALARIO">CONTA SALÁRIO</option></select>
                {metodoSelecionado === 'SEI' && <input name="numero_processo" placeholder="Nº Processo SEI" className="border-2 border-blue-100 p-3 sm:p-4 rounded-xl bg-blue-50 text-sm font-bold text-blue-800 uppercase outline-none focus:ring-2 focus:ring-blue-500" onInput={(e) => e.currentTarget.value = e.currentTarget.value.toUpperCase()} />}
                <textarea name="observacoes" placeholder="Observações..." className="border p-3 sm:p-4 rounded-xl bg-slate-50 text-sm resize-none outline-none focus:ring-2 focus:ring-blue-500" rows={2} />
                <div className="flex items-center gap-2 px-1 py-2">
                  <input type="checkbox" id="manterDados" checked={manterDados} onChange={(e) => setManterDados(e.target.checked)} className="w-5 h-5 cursor-pointer accent-slate-900 rounded" />
                ...
                <button className="bg-slate-900 text-white font-black py-4 rounded-xl uppercase text-xs shadow-lg hover:bg-black transition-all active:scale-95 mt-2">Salvar Diária</button>
              </form>
            </div>
          </aside>
                  ...
                  <div key={item.id} className={`bg-white p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] shadow-sm border-l-[8px] sm:border-l-[12px] transition-all duration-300 ${item.pago ? 'border-green-500 bg-slate-50 opacity-90 hover:opacity-100' : 'border-red-500 shadow-md transform hover:-translate-y-1'}`}>
                    {idEditando === item.id ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 relative">
                        <div className="col-span-1 sm:col-span-2 text-[10px] font-black text-blue-600 mb-2 italic">EDITANDO REGISTRO</div>
                        <input value={dadosEditados.nome || ""} onChange={e => setDadosEditados({...dadosEditados, nome: e.target.value.toUpperCase()})} className="border p-3 rounded-lg text-sm uppercase" placeholder="Nome" />
                        <input value={dadosEditados.adolescente_nome || ""} onChange={e => setDadosEditados({...dadosEditados, adolescente_nome: e.target.value.toUpperCase()})} className="border p-3 rounded-lg text-sm uppercase" placeholder="Adolescente" />
                        <input type="date" value={dadosEditados.data_viagem || ""} onChange={e => setDadosEditados({...dadosEditados, data_viagem: e.target.value})} className="border p-3 rounded-lg text-sm" />
                        <input value={dadosEditados.local_viagem || ""} onChange={e => setDadosEditados({...dadosEditados, local_viagem: e.target.value.toUpperCase()})} className="border p-3 rounded-lg text-sm uppercase" placeholder="Destino" />
                        <input value={dadosEditados.numero_processo || ""} onChange={e => setDadosEditados({...dadosEditados, numero_processo: e.target.value.toUpperCase()})} className="border p-3 rounded-lg text-sm uppercase" placeholder="Processo" />
                        <input type="number" value={dadosEditados.valor || ""} onChange={e => setDadosEditados({...dadosEditados, valor: e.target.value})} className="border p-3 rounded-lg text-sm" placeholder="Valor" />
                        <input value={dadosEditados.cargo || ""} onChange={e => setDadosEditados({...dadosEditados, cargo: e.target.value.toUpperCase()})} className="border p-3 rounded-lg text-sm uppercase" placeholder="Cargo" />
                        <input value={dadosEditados.quantidade || ""} onChange={e => setDadosEditados({...dadosEditados, quantidade: e.target.value})} className="border p-3 rounded-lg text-sm" placeholder="Qtd" />
                        <textarea value={dadosEditados.observacoes || ""} onChange={e => setDadosEditados({...dadosEditados, observacoes: e.target.value})} className="col-span-1 sm:col-span-2 border p-3 rounded-lg text-sm" placeholder="Observações" />
                        <div className="col-span-1 sm:col-span-2 flex flex-col sm:flex-row gap-2 mt-2">
                          <button onClick={salvarEdicao} className="w-full sm:w-auto flex-1 bg-green-600 text-white p-4 sm:p-3 rounded-xl font-bold uppercase text-[10px]">Salvar Alterações</button>
                          <button onClick={() => setIdEditando(null)} className="w-full sm:w-auto flex-1 bg-slate-200 text-slate-600 p-4 sm:p-3 rounded-xl font-bold uppercase text-[10px]">Cancelar</button>
                        </div>
                      </div>
                    ) : (
                        ...
                        <div className="flex-1">
                          <div className="flex flex-wrap items-center gap-2 mb-3 sm:mb-2">
                            ...
                            {item.data_ultima_exportacao && !item.pago && (
                              <span className="text-[9px] font-bold text-amber-600 bg-amber-50 border border-amber-200 px-2 py-1 rounded-md uppercase">📥 Gerada</span>
                            )}
                          </div>
                          
                          <h3 className="font-black text-slate-800 uppercase text-base sm:text-lg leading-tight">{item.nome}</h3>
                        ...
                        <div className="sm:text-right flex flex-col justify-between items-start sm:items-end border-t sm:border-t-0 pt-4 sm:pt-0 mt-2 sm:mt-0">
                          <div className="text-left sm:text-right w-full sm:w-auto flex flex-row sm:flex-col justify-between items-center sm:items-end">
                             ...
                             <div className="text-[8px] sm:text-[9px] text-slate-400 sm:mt-2 space-y-0.5 text-right">
                               {item.created_at && <p className="flex items-center justify-end gap-1">➕ {new Date(item.created_at).toLocaleDateString('pt-BR')} <span className="bg-slate-100 text-slate-500 px-1 rounded font-bold hidden sm:inline-block">{item.usuario_alteracao ? `por ${item.usuario_alteracao}` : ''}</span></p>}
                          ...
                          {isAdmin && (
                            <div className="flex gap-2 mt-4 w-full sm:w-auto">
                              <button onClick={() => alternarPagamento(item.id, item.pago)} className={`flex-1 sm:flex-none px-3 py-3 sm:py-2.5 rounded-xl font-black text-[10px] uppercase transition-all shadow-sm ${item.pago ? 'bg-slate-200 text-slate-500 hover:bg-red-100 hover:text-red-600' : 'bg-green-600 text-white hover:bg-green-700'}`}>{item.pago ? 'DESMARCAR' : 'MARCAR PAGO'}</button>
                              <button onClick={() => iniciarEdicao(item)} className="bg-slate-100 sm:bg-white border-2 border-slate-100 px-4 py-3 sm:py-2.5 rounded-xl hover:bg-blue-50 text-slate-400 hover:text-blue-600 transition-colors">✏️</button>
                              <button onClick={() => excluirDiaria(item.id)} className="bg-slate-100 sm:bg-white border-2 border-slate-100 px-4 py-3 sm:py-2.5 rounded-xl text-slate-300 hover:text-red-500 hover:bg-red-50 transition-colors">🗑️</button>
                            </div>
                          )}
                        </div>
                      </div>
